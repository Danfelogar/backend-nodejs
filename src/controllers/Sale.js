...
let query = async(req,res,next) =>{
    ...
    try {
        const data = await models.Sale.findOne({ _id: id })
...
let list = async(req,res,next) =>{
    ...
    try {
        const data = await models.Sale.find({
...
let activate = async(req, res, next) =>{
    ...
    try {
        const data = await models.Sale.findOneAndUpdate({_id:id},{state:1})
...
let desactivate = async(req, res, next) =>{
    const id = req.body._id;
    try {
        const data = await models.Sale.findOneAndUpdate({_id:id},{state:0})
        let details = data.detail;
        details.map((x)=>{
...
let annualGraph = async(req, res, next) =>{
    try {
        const data = await models.Sale.aggregate([
            {
                $group:{
                    _id: {
